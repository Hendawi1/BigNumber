# Подробный и полный рапорт (write-up) по проекту: реализация класса `BigNumber` для работы с большими целыми числами
...
* **Основание**: выбор (b) определяет плотность хранения и простоту операций с переносом. Часто выбирают степень двух $(2^8, 2^{16}, 2^{32})$.
...
`coefs[0]` — младший байт $256^0$
`coefs[n-1]` — старший. Trim удаляет ведущие нули, оставляя по крайней мере один лимб (для нуля — один нулевой лимб).
...
* Формально: $A == B \iff \text{len}(A) == \text{len}(B) \land \forall i \in [0..\text{len}-1]: A.\text{coefs}[i] == B.\text{coefs}[i]$
...
* Для i от m до 0:
    ...
    * Вычисляем пробное частное: $\hat{q} = \left\lfloor \frac{u_2 \cdot b + u_1}{v_1} \right\rfloor, \quad \hat{r} = (u_2 \cdot b + u_1) \bmod v_1$

  
    * Коррекция: пока $(\hat{q} = b)$ или $(\hat{q} \cdot v_2 > \hat{r}\cdot b + u_0)$, уменьшаем $\hat{q}$ и увеличиваем $\hat{r}$ на `v1`. Если $\hat{r} \ge b$ — прерываем.
